...
| C05 | LowonganPekerjaan | idLowongan, judul, deskripsi, kuota, lokasi, upah, status | validasiIsianFormulir(), simpanLowongan(), setStatusDefault() |
...
| C05 | LowonganPekerjaan | idLowongan, judul, kategori, lokasi, upah, status | tampilkanDaftarLowongan(filter), cekStatusOpen() |
...
| C05 | LowonganPekerjaan | idLowongan, judul, status | tampilkanTombolAjukan() |
| C06 | PengajuanPenawaran | idPenawaran, idLowongan, idTenagaKerja, pesanPenawaran, timestamp, status | simpanPengajuan(), validasiStatusAkun() |
| C14 | Notifikasi | idNotifikasi, idPenerima, pesan, statusBaca | kirimNotifikasiPengajuanBaru() |
